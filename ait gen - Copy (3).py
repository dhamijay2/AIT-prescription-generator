...
# --- Database Setup ---
DB_FILE = os.path.join(WORKING_DIR, "UC_Allergy_AIT.db")
    ...
    try:
        ...
        if existing_patient:
            ...
            cursor.execute('''
                ...
                (patient_name, dob, mrn, address, city, state, zip_code, phone, allergens, treatment_type)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', (
                patient_data['patient_name'],
                patient_data['dob'].strftime("%m-%d-%Y"),
...
def load_patient():
    ...
    def load_selected_patient():
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.execute('''
                SELECT patient_name, dob, mrn, address, city, state, zip_code, phone, allergens, treatment_type
                FROM patients
            ...
            if result:
                patient_name, dob, mrn, address, city, state, zip_code, phone, allergens_str, treatment_type = result
...
def generate_vials(selected_allergens):
...
def generate_prescription():
    ...
    vials = generate_vials(selected_allergens)
    ...
    dilutions = get_dilutions(mode, treatment_type)
    ...
    prescription_text += f"City: {city}, {state} {zip_code}\n"
    ...
    for vial in vials:
        ...
        prescription_text += f"  • Diluent: {vial.remaining_volume():.2f} mL\n"
...
def export_prescription_to_pdf():
    ...
    try:
        ...
        for vial in last_vials:
            ...
            vial_contents.append(["Diluent:", f"{vial.remaining_volume():.2f} mL"])
...
def generate_labels_pdf(file_path):
    ...
    try:
        ...
        for label_info in label_data_list:
            ...
            text_y = y + label_height - 0.08 * inch - text_offset
...
title_label = tk.Label(title_frame, text="Allergy Shot Vial Prescription Generator", 
...
export_pdf_button = ttk.Button(button_frame1, text="📄 Print Script", command=export_prescription_to_pdf)
...
stock_mgmt_button = ttk.Button(button_frame2, text="🏷️ Stock Management", command=show_stock_management)
